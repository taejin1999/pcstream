...
We compared WAF of the existing techniques with \small\textsf{PCStream} for the three
scenarios, and the result is shown in Fig.~\ref{fig:result_emul}.  As shown in
Fig.~\ref{fig:result_emul}, \textsf{PCStream$^*$} reduced WAF by up to 30\% over
...
effective in reducing WAF.  Moreover, \textsf{PCStream} even showed similar WAF to
...
In order to analyze the improvement made by \textsf{PCStream}, 
...
the hot stream.
On the other hand, \textsf{PCStream$^*$} can identify short-lived data from 
...
commercial SSD firmware, we evaluated \textsf{PCStream$^*$} only.  To warm up the
SSD before running benchmarks, we filled up 90\% of the SSD capacity with valid
data.

As illustrated in Fig.~\ref{fig:result_SSD}, \textsf{PCStream$^*$} reduced WAF by
