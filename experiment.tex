...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.9\linewidth]{figure/result_emul.pdf}
	\vspace{-10pt}
...
\begin{figure}[b]
	\centering
	\includegraphics[width=0.9\linewidth]{figure/streamlifetime}
	\vspace{-10pt}
%	\caption{The WAF comparison on the emulator.}
	\caption{A lifetime distribution of stream data}
...
Fig.~\ref{fig:streamlifetime} shows how big is the difference of the 
lifetime of data allocated to the same stream by each technique.
As shown in Fig.~\ref{fig:streamlifetime}, for AutoStream, data in the same stream show
large difference since AutoStream does not notice when long-lived data is written to
the hot address area.
However, {\sf PCStream$^*$} can identify short-lived data from 
several dominant I/O activities as mentioned 
in section 2, e.g., S0 and S2 in the Fig.~\ref{fig:streamlifetime}.
The long-lived data moved to a substream by {\sf PCStream} did not erased
during the execution so the lifetime of data in the substream, e.g. S6, S7,
can not be measured.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.9\linewidth]{figure/result_ssd.pdf}
	\vspace{-10pt}
